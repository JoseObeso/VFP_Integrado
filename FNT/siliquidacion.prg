***********************************************************************************************************
********  SISTEMA  INFORMATICO  DE   LIQUIDACIONES - CONEXION FARMACIA - CUENTAS - EMERGENCIA  ************************
************************************************************************************************************
****************     SILIQUIDACIONES    ************************************************************************

PUBLIC gconecta, gctitulo, gcfecha, gchora, gctemporal, gcgraficos, lrecupera, lcempresa, lcdepartamento, lclogo, lcuser,;
       lcentidade, lcruce, lcdireccione, lcfonoe, lcunidade, lcrepresentantee, lclogoe, lccorreoe, lctitulo,   gcunidad, gndecimal, lcmenu,  gcnombrepc_user_red
...
IF FirsTime("siliquidacion.exe")  = .t.
...
IF Archivo_ = .T. 
      ...
      ENDIF
       gcruta = lcruta       
      gcsistema = "LIQUIDACION"
       gcnombrepc_user_red = SYS(0)
      gctitulo = "SISTEMA INFORMATICO OPERACIONES EN LIQUIDACION - REPORTES"
      gndecimal = 0
      _screen.Caption = gcsistema + " : " + gctitulo +  "  [FECHA : " + gcfecha + "] " + "  APLICACION : " + gcruta + "   SERVIDOR :   " + UPPER(x_Server)
      _screen.picture=gcgraficos + "fondo_liquidaciones.jpg"
      _screen.icon=gcgraficos  + "wsi.ico"
  ...
  ELSE
      ...
      DO FORM frmlogin_liquidaciones.scx
      READ events
  ENDIF
  ON SHUTDOWN  
  DO ambientador
