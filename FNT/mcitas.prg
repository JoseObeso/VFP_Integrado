************* APLICACION PARA MONITOREO DE CONSULTORIOS*
*****************************************************************************
...
* REVISA LA EJEUCION POR PRIMERA VEZ 

IF FirsTime("mcitas.exe")  = .t.
   ** Leer del INI
   ...
   IF Archivo_ = .T. 
      ...
      gcruta = lcruta       
      gcsistema = "MCITAS"
      gctitulo = "SISTEMA DE MONITOREO DE CONSULTORIOS "
      gndecimal = 0
      gcnombrepc_user_red = SYS(0)
      _screen.Caption = gcsistema + " : " + gctitulo +  "  [FECHA : " + gcfecha + "] " + "  APLICACION : " + gcruta + "   SERVIDOR :   " + UPPER(x_Server)
      _screen.picture=gcgraficos + "fondocitas.jpg"
      _screen.icon=gcgraficos  + "cero.ico"
  ...
  FOR RT = 1 TO lnbucle
    ...
    Wait Window cMensage At Int(_Screen.Height/2), Int(_Screen.Width/2 - Len(cMensage)/2) nowait  
      TEXT TO lqry_agregar_citas noshow
         declare @lnanio_server int = RIGHT(?lnanio,2)
         declare @lid_cita_menor int = (select convert(int, MAX(CITA_ID)) + 1  as id_cita from [SIGSALUD].[dbo].[CITA] where SUBSTRING(CITA_ID,1,2) = @lnanio_server)
         declare @lid_cita_ver_anio INT  = CONVERT(INT, SUBSTRING(convert(varchar(10), (select convert(int, MAX(CITA_ID)) + 1  as id_cita from [SIGSALUD].[dbo].[CITA])),1,2))
         declare @lid_cita_cotinua int = (select convert(int, MAX(CITA_ID)) + 1  as id_cita from [SIGSALUD].[dbo].[CITA])
         declare @lid_cita_nuevo int = 210000000
         declare @lid_cita int = (select case when @lnanio_server > @lid_cita_ver_anio then @lid_cita_nuevo when @lnanio_server =  @lid_cita_ver_anio then @lid_cita_cotinua else @lid_cita_menor end)         
...
FUNCTION agregar_citas_con_horas_n() 
  FOR RT = 1 TO lnbucle_noche
    lcnumerocita = PADL(ALLTRIM(STR(lnnumeroinicial)), 2, '0')
    lhoragrabar =  lchora_inicio
    cMensage = '........AGREGANDO CITA NRO. :  ' +lcnumerocita
    _Screen.Scalemode = 0
    Wait Window cMensage At Int(_Screen.Height/2), Int(_Screen.Width/2 - Len(cMensage)/2) nowait  
      TEXT TO lqry_agregar_citas_horas noshow
         ...
         DECLARE @ldfecha datetime = convert(datetime, ?lcfechadia, 101)
         declare @lchora char(6) = ?lhoragrabar
         DECLARE @ldfecha_nuevafecha datetime = case when @lchora between '00:00' and  '08:00' then  DATEADD(DAY,1,@ldfecha)  else @ldfecha end 
         declare @lcnumero varchar(3) = ?lcnumerocita
         declare @lcturno char(2) = ?lcturnon
         declare @lcnombre varchar(60) = '                                                              '
         DECLARE @lcconsultorio varchar(8) = ?lcconsultorio
         declare @lcmedico varchar(4) = ?lcmedico
         INSERT INTO [SIGSALUD].[dbo].[CITA]([CITA_ID],[CONSULTORIO],[MEDICO],[FECHA_PROGRAMACION],[FECHA_OTORGA],[FECHA_PAGO],[FECHA_LIBRE],[FECHA],[HORA],[TURNO_CONSULTA],[TIPO_PACIENTE],[TIPO_CITA],[PACIENTE],[NOMBRE],[OBSERVACION],
           [PAGOID],[SEGURO],[ESTADO],[NUMERO],[HORA_OTORGA],[TIPO_SOLICITUD],[SITUACION],[USUARIO],[NUMATENCION],[USER_ELIMINACION],[FECHA_HORA_ELIMINACION])
                 VALUES (convert(char(9), @lid_cita), @lcconsultorio, @lcmedico, GETDATE() - 0.15, NULL, NULL, NULL, @ldfecha_nuevafecha, @lchora, @lcturno, 'N', 'C', ' ', ' ', ' ', ' ','0','1', @lcnumero, @lcnombre, 'P', null, null, '0', null, null)
      ENDTEXT
      lejecutabusca = sqlexec(gconecta,lqry_agregar_citas_horas)
      lnnumeroinicial =   lnnumeroinicial  +1
