...
lcfechafin =  '2018'  + '-'  +  '03'  + '-' + '27'
lcuser = "Jobeso"

lcfechainicio_mostrar = '28' + '/' + '02' + '/' + '2018'
lcfechafin_mostrar =  '27' + '/' + '03' + '/' + '2018'
...
TEXT TO lobtener_items noshow
  ...
  select b.CLASE, b.NOMBRE as NOMBRE_CLASE, a.ITEM, A.interfase2 AS SISMED, UPPER(a.NOMBRE) as nombre, UPPER(A.PRESENTACION) AS PRESENTACION, GETDATE() AS DFECHA_INICIO_PROCESO
	     from [SIGSALUD].[dbo].[ITEM] a   left join [SIGSALUD].[dbo].[CLASE] b on a.CLASE = b.CLASE where  SUBSTRING(A.item,1,2) = '17' and A.ITEM in ('170618')
...
ln_stock_coche = litem_stock_farmacia_so + litem_stock_farmacia_ci + litem_stock_farmacia_go +    litem_stock_farmacia_pe + litem_stock_farmacia_me + litem_stock_farmacia_ne + litem_stock_farmacia_uc +    litem_stock_farmacia_am + litem_stock_farmacia_em 
****
...
TEXT TO lqry_ver_cp_anulados_f noshow
  ...
  select SUM(CANTIDAD) as DEVO_CREDITO_PACIENTE    from [SIGSALUD].[dbo].[V_KARDEX] a left join USUARIO b on b.USUARIO = a.USUARIO  where OPERACION between @opeinicio and @opefin and ITEM = @lcitem  and ALMACEN = 'F' AND
   B.MODULO = 'FARMACIA'  AND TIPO_TRANSACCION IN ('IAN')  AND IDTRANSACCION in (select IDTRANSACCION  from [SIGSALUD].[dbo].[V_KARDEX] a left join USUARIO b on b.USUARIO = a.USUARIO  where OPERACION between @opeinicio and @opefin and ITEM = @lcitem  and ALMACEN = 'F' AND
...
TEXT TO lqry_ver_cp_anulados_c noshow
  ...
  select SUM(CANTIDAD) as DEVO_CREDITO_PACIENTE    from [SIGSALUD].[dbo].[V_KARDEX] a left join USUARIO b on b.USUARIO = a.USUARIO  where OPERACION between @opeinicio and @opefin and ITEM = @lcitem  and ALMACEN = 'C' AND
   B.MODULO = 'FARMACIA'  AND TIPO_TRANSACCION IN ('IAN')  AND IDTRANSACCION in (select IDTRANSACCION  from [SIGSALUD].[dbo].[V_KARDEX] a left join USUARIO b on b.USUARIO = a.USUARIO  where OPERACION between @opeinicio and @opefin and ITEM = @lcitem  and ALMACEN = 'C' AND
   B.MODULO = 'FARMACIA'  AND TIPO_TRANSACCION IN ('VRP'))  group by ITEM  
ENDTEXT
